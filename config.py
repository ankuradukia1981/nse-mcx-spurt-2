...
# v6 spec: default spike threshold is 10% (was 5% in earlier versions).
...
# Option-chain responses are cached this long before re-hitting Dhan, to stay
# well under the ~1 request / 3s per-underlying rate limit even with several
# instruments refreshing on the same auto-refresh tick.
CHAIN_CACHE_TTL_SECONDS = 45

# Where previous-day baseline snapshots (spot/expiry/strike/CE/PE/combined
# premium, one per symbol per date) are persisted between runs/days.

...
- Per-row LIVE/SIM source tag — one symbol can be live while another
  quietly falls back to simulation (rate limit, unresolved id, etc.).
- PCR (chain-wide Put/Call OI ratio) and ATM delta alongside CE/PE/IV.
- A Stats tab (session high/low/avg premium, spike counts) next to Alerts.
- "Show only ≥ threshold" watchlist filter, and a Pause/Resume control
